(function ($) {
	...
	var getCurrentFooter = function (that) {
		var footer = that.$tableFooter;
		return footer;
	};
	...
	var setFooter = function (that) {
		var footer = getCurrentFooter(that)
		let tr = footer.children().children().children().children()
		var currentColumns = [];
		...
		if (that.formatNoMatches) {
			$(that.$tableBody[0]).height(that.options.height - 121);
		}
		$.each(tr, function (i, td) {
			let column = currentColumns[i];
			td = $(td);
			if (that.formatNoMatches && currentColumns[i].width) {
				td.width(currentColumns[i].width)
			}
			if (i === 0) {
				let icon = '<span class="iconfont icon-xinjian addIcon" id="addIcon-' + that.$el[0].id + '"></span>'
				td.find('.th-inner').html(icon);
				$('#addIcon-' + that.$el[0].id).on('click', function () {
					if (urlstr === "" || urlstr === null || urlstr === undefined) {
						$('#promptLessOneModal').modal('show');
						return;
					}
					let data = {}
					let url = that.$body.context.getAttribute('data-addUrl');
					$.each(tr, function (i, td) {
						let currentColumn = currentColumns[i];
						if (currentColumn.type !== undefined) {
							let currentTd = $(td);
							if (currentTd.find('.th-inner').children().val() === "是") {
								data[currentColumn.field] = 1;
							} else if (currentTd.find('.th-inner').children().val() === "否") {
								data[currentColumn.field] = 0;
							} else {
								data[currentColumn.field] = currentTd.find('.th-inner').children().val();
							}
						}
					});
					data["lexiconId"] = urlstr;
					that.trigger('click-footer-add', data);
				})
			} else if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'input') {
				let input = '<input type="text" class="addNewInfoInput ' + column.field + '">'
				td.find('.th-inner').html(input);
			} else if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'select') {
				let select = '<select class="addNewSelect ' + column.field + '"><option value="0">否</option><option value="1">是</option></select>'
				td.find('.th-inner').html(select);
			} else if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'datepicker' && column.footerAdd === true) {
				let datepicker = '<input type="text" class="addNewTime date-filter-control bootstrap-table-filter-control-' + column.field + '">'
				td.find('.th-inner').html(datepicker);
				footer.find('.date-filter-control.bootstrap-table-filter-control-' + column.field).datepicker(column.filterDatepickerOptions)
	...
	var createControls = function (that, header) {
		...
		$.each(header.children().children(), function (i, tr) {
			tr = $(tr);
			var currentCell = tr.find('.fht-cell');
			filterTh += '<th data-field="' + tr.data('field') + '">' + currentCell.html() + '</th>'
			currentCell.html('')
		});
		header.append('<tr>' + filterTh + '</tr>')
		if (addedFilterControl) {
			header.off('keyup', 'input').on('keyup', 'input', function (event) {
				if (this.getAttribute('comStart') === 'start') return;
				clearTimeout(timeoutId);
				timeoutId = setTimeout(function () {
					that.onColumnSearch(event);
				}, that.options.searchTimeOut);
			}).on('compositionstart', 'input', function () {
				this.setAttribute('comStart', 'start');
			}).on('compositionend', 'input', function () {
				this.setAttribute('comStart', 'false');
			});
	...
	$.extend($.fn.bootstrapTable.Constructor.EVENTS, {
		...
		'click-footer-add.bs.table': 'onClickFooterAdd'
	});
